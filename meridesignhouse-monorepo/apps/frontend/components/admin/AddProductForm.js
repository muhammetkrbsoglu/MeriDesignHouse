...
export default function AddProductForm({ categories }) {
  ...
  const [formData, setFormData] = useState({
    ...
    categoryId: "",
    featured: false,
    isPopular: false, // ✅ Added for Popular Products
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3001'}/api/product`, {
        method: "POST",
      ...
      if (response.ok) {
        ...
        setErrors({ submit: errorData.error || "Ürün oluşturulamadı" })
      }
    } catch (error) {
